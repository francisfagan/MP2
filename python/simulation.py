...
class Simulation:
    def __init__(self, bodies: list[Body]) -> None:
        ...
        self.energy_values: np.ndarray | None = None
        self.spheres = []
        self.scene : canvas
    ...
    # Calculates total mechanical energy for one complete state matrix.
    # First three columns are position; last three columns are velocity.
    def total_energy(self, state: np.ndarray) -> float:
        pos = state[:, :3]
        vel = state[:, 3:]

        kinetic_energy = 0.5 * np.sum(
            self.masses * np.sum(vel ** 2, axis=1)
        )

        diff = pos[None, :, :] - pos[:, None, :]
        distance = np.sqrt(np.sum(diff ** 2, axis=-1))

        unique_pairs = np.triu_indices(self.N_bodies, k=1)
        mass_pairs = np.outer(self.masses, self.masses)[unique_pairs]

        potential_energy = -G * np.sum(
            mass_pairs / distance[unique_pairs]
        )

        return kinetic_energy + potential_energy


    # Returns relative change in total energy after run(record_energy=True).
    def relative_energy_drift(self) -> np.ndarray:
        if self.energy_values is None:
            raise ValueError(
                "Energy was not recorded. Run the simulation with record_energy=True."
            )

        initial_energy = self.energy_values[0]
        return (self.energy_values - initial_energy) / abs(initial_energy)


    # Main method called for running sim.
    #   -Calculates body positions one time step forward using chosen method
    #   -Stores position at each iteration for plotting and animation
    #   -Optionally records total energy for method accuracy comparison
    def run(self, dt: float, T: float, t0: float = 0.0,
            method: str = "abm4", record_energy: bool = False) -> None:
        n_steps = int((T - t0) / dt)
        history = [self.mat[:, :3].copy()]
        energies = [self.total_energy(self.mat)] if record_energy else None
        t = t0
        ...
        if method == "rk4":
            for step in tqdm(range(n_steps)):
                ...
                if record_energy:
                    energies.append(self.total_energy(self.mat))
            ...
            for y in (y2, y3, y4, y5):
                ...
                if record_energy:
                    energies.append(self.total_energy(y))
            ...
            for step in tqdm(range(n_steps - 4)):
                ...
                if record_energy:
                    energies.append(self.total_energy(y_new))
        ...
        self.energy_values = np.array(energies) if record_energy else None
