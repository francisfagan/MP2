...
def main():
    ...
    for m in methods:
        ...
        sim.run(args.dt, T, 0.0, m, record_energy=True)

        if sim.energy_values is None:
            raise RuntimeError("Energy recording failed.")

        indices = np.arange(0, len(sim.energy_values), args.sample_every)
        E = sim.energy_values[indices]
        t_arr = indices * args.dt
